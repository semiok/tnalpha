...
每个 prompt 函数都接入 resolve 机制：用户可在提示词管理页面修改，
运行时优先使用覆盖版本，恢复默认则回退到下面的代码。
"""
...
def content_analysis(filename: str, text: str, has_attachment: bool = False) -> str:
    ...
    return resolve("knowledge:content_analysis", default,
                   attachment_note=attachment_note, filename=filename, text=text)
...
def style_analysis(filename: str) -> str:
    ...
    return resolve("knowledge:style_analysis", default, filename=filename)


def aggregate_content(items: list[tuple[str, str]]) -> str:
    body = "\n\n".join("【{name}】\n{a}".format(name=name, a=a) for name, a in items)
    default = (
        "综合以下各文档解读，输出该品牌主题的**综合内容定义**（中文，供选题与文章生成统一参考）："
        "可用主题方向、核心素材、调性要点。清晰完整、信息密度高。\n\n{body}"
    )
    return resolve("knowledge:aggregate_content", default, body=body)


def aggregate_style(items: list[tuple[str, str]]) -> str:
    body = "\n\n".join("【{name}】\n{s}".format(name=name, s=s) for name, s in items)
    default = (
        "综合以下各文档的视觉风格解读，输出该品牌的**统一视觉风格指南**（中文，供 AI 配图参考）。\n\n{body}"
    )
    return resolve("knowledge:aggregate_style", default, body=body)


def brand_fields_prompt(brand_name: str, doc_digest: str) -> str:
    default = (
        "以下是「{brand_name}」品牌的内容解读（已从资料蒸馏）。"
    ...
    return resolve("knowledge:brand_fields_prompt", default,
                   brand_name=brand_name, doc_digest=doc_digest or "")
